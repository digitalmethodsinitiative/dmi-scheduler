...
class Job:
	...
	def get_by_remote_ID(remote_id, database, pythonfile="*"):
		"""
		...
		:param database: Database handler
		:param str pythonfile: Job type
		:param str remote_id: Job remote ID
		:return Job: Job object
		"""
		if pythonfile != "*":
			data = database.fetchone("SELECT * FROM jobs WHERE pythonfile = %s AND remote_id = %s", (pythonfile, remote_id))
		else:
	...
	def claim(self):
		...
		updated = self._db.update("jobs", data={"timestamp_claimed": claim_time, "timestamp_lastclaimed": claim_time},
								  where={"pythonfile": self.data["pythonfile"], "remote_id": self.data["remote_id"],
										"timestamp_claimed": 0})
	...
	def finish(self, delete=False):
		...
		if self.data["interval"] == 0 or delete:
			self._db.delete("jobs", where={"pythonfile": self.data["pythonfile"], "remote_id": self.data["remote_id"]})
		else:
			self._db.update("jobs", data={"timestamp_claimed": 0, "attempts": 0},
							where={"pythonfile": self.data["pythonfile"], "remote_id": self.data["remote_id"]})
	...
	def release(self, delay=0, claim_after=0):
		...
		self._db.update("jobs", data=update,
						where={"pythonfile": self.data["pythonfile"], "remote_id": self.data["remote_id"]})
	...
	def update_status(self, status):
		...
		self._db.update("jobs", data={"status": status},
						where={"pythonfile": self.data["pythonfile"], "remote_id": self.data["remote_id"]})
